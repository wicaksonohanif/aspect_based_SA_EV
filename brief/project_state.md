...
- Memperbaiki aturan berbasis negasi lokal (*explicit negative words* seperti "murahan", "kurang nyaman") dan pemetaan fitur/desain/atap ke aspek Kualitas & Durabilitas (`kualitas`).
...
- Mengintegrasikan hasil audit manusia sebagai *Ground Truth Override* di atas label mesin.
...
1. **Penyusunan Spesifikasi Phase 3 (`specs/03_model_training.spec.md`):**
   - Merancang eksperimen arsitektur model baseline (TF-IDF + SVM / Random Forest / Logistic Regression) & Deep Learning / Transformer (IndoBERT / IndoRoBERTa / Fine-Tuning).
